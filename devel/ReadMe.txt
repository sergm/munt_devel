...
+ most probably is the technology used in the LA h/w
+ can surely be implemented in hardware using cosine LUT for speedup computations (h/w is suspected to use logarithmic cosine LUT to get rid of multiplications)
+ the LUT is the only memory needed for WG
+ waves produced are perfect and unique
+ this makes mindful use of the cosine LUT to form sawtooth waves and for resonance emulation as well
+ gives waveforms exactly the same as a LA chip
- can be claimed as Casio's patent violation when implemented in a electronic musical instrument
- for now, it's unknown in resonance emulation how the resonance sine fades (linear approx. used for awhile)